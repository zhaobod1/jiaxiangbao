...
//    }
    ...
    if(count($info)>0){
        ...
        $insql = "INSERT INTO " . $ecs->table('supplier_admin_user') . " (`uid`, `user_name`, `email`, `password`, `ec_salt`, `add_time`, `last_login`, `last_ip`, `action_list`, `nav_list`, `lang_type`, `agency_id`, `supplier_id`, `todolist`, `role_id`, `checked`) ".
					"VALUES(".$supplier_old['user_id'].", '".$supplier_old['user_name']."', '".$supplier_old['email']."', '".$supplier_old['password']."', '".$supplier_old['ec_salt']."', ".$supplier_old['last_login'].", ".$supplier_old['last_login'].", '".$supplier_old['last_ip']."', 'goods_list,category_list,comment_manage,goods_trash,goods_manage,remove_back,cat_manage,cat_drop,order_comment_priv,rebate_manage,order_list,order_query,merge_order,edit_order_print,delivery_order,back_order,order_os_edit,order_ps_edit,order_ss_edit,order_edit,booking,shop_base,shop_menu,shop_guanggao,shop_article,shop_header,shop_templates,ship_manage,admin_list,admin_manage,admin_drop,allot_priv,bonus_manage,group_by,favourable,auction,pre_sale,pickup_point_manage,pickup_point_batch,customer,virtual_card_list,virtual_goods_s,virtual_validate,virtual_goods_add', '', '', 0, ".$supplier_old['supplier_id'].", NULL, NULL, ".intval($_POST['status']).")";
        $db->query($insql);
    }
...
function suppliers_list()
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$row['user_id'] = $GLOBALS['db']->getOne("select user_id from ".$GLOBALS['ecs']->table("supplier_admin_user")." where supplier_id=".$row['supplier_id']);//yxn
